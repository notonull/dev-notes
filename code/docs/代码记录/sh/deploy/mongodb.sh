...
get_service_urls() {
    ...
    for port_mapping in "${env_ports[@]}"; do
        ...
        urls+=("mongodb://${base_ip}:${host_port}")
    done
    printf '%s\n' "${urls[@]}"
}
...
generate_compose_file() {
    # 验证配置
    if ! validate_config_arrays "MongoDB" "env_ports" "env_volumes" "env_environment"; then
        return 1
    fi
